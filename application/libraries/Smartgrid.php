        ...
        switch($field_type){
            ...
            case "status":
              if($field_value == 1){
                ...
                $html .= '<span class="badge badge-secondary">Inactiva</span>';
              }
   ...
   */   
    private function get_relative_date($ts) 
    {
        ...
        if($diff == 0) {
            ...
            if($day_diff < 31) { $week = ceil($day_diff / 7); return 'Hace ' . $week . ' semana'.(($week == 1) ? '' :'s'); }
            if($day_diff < 60) { return 'last month'; }
            return $this->get_date_spanish($ts);
        } else {
            $diff = abs($diff);
            $day_diff = floor($diff / 86400);
            if($day_diff == 0) {
                if($diff < 120) { return 'En 1 minuto'; }
                if($diff < 3600) { return 'En ' . floor($diff / 60) . ' minutos'; }
                if($diff < 7200) { return 'En 1 hora'; }
                if($diff < 86400) { return 'En ' . floor($diff / 3600) . ' horas'; }
            }
            if($day_diff == 1) { return 'Mañana'; }
            if($day_diff < 4) { return date('l', $ts); }
            if($day_diff < 7 + (7 - date('w'))) { return 'La próxima semana'; }
            if(ceil($day_diff / 7) < 4) { $week = ceil($day_diff / 7); return 'En ' . $week . ' semana'.(($week == 1) ? '' :'s'); }
            if(date('n', $ts) == date('n') + 1) { return 'El próximo mes'; }
            return $this->get_date_spanish($ts);
        }
    }
    
    function get_date_spanish($ts){
      $ts = (!ctype_digit($ts)) ? strtotime($ts) : $ts;
      $meses = array("Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
      return date('j', $ts)." de ".$meses[date('n', $ts) - 1]. " del ".date('Y', $ts) ;
    }
